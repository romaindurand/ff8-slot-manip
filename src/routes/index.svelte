<script>
  ...
  const blackDots = [43, 79, 91, 175, 179, 187, 207, 231, 251]
    .sort((a, b) => a > b);
  
  $: computedTable = RNGMap
  .map(row => {
    return {
      ...row,
  ...
  .map(row => {
    const spellName1 = spellTable?.[row.table - 1]?.[row.current_crisis_level - 1]?.[row.entry - 1]
    const rowEntryModulo = spellTable?.[row.table - 1]?.[row.current_crisis_level - 1]?.[row.entry] ? row.entry : row.entry - 256
    const spellName2 = spellTable?.[row.table - 1]?.[row.current_crisis_level - 1]?.[rowEntryModulo]
    const spellName3 = spellTable?.[row.table - 1]?.[row.current_crisis_level - 1]?.[rowEntryModulo + 1]
    return {
      ...row,
  ...
  function computeCrisisLevel(random_mod, currentHp) {
    ...
    const limitLevel = Math.floor((statusBonus + deathBonus - hpMod) / (random_mod + 160));
  ...
  $: spellList = [selectedSpell1, selectedSpell2, selectedSpell3].filter(spell => spell)
  ...
  function computeManip({random_mod, rng, table, entry, current_crisis_level}, spellOrder) {
    const currentRng = (rng + spellOrder * 4) % 256
    if (table === 4 && current_crisis_level === 4) {
      let delta = 183 - currentRng
      if (delta < 0) delta += 256
      return `Do over: ${delta / 4}`
    }
    ...
    const closestBlackDot = blackDots.find((blackDot) => {
      return blackDot >= rng;
    }) || blackDots[0];
    ...
    if (table === 4) {
      ...
      return `Do over ${doOver1} then skip turn ${skipTurn} then do over ${doOver2}`
    }
...
<p>Currently Working only on lvl 8, with max Hp at 482, targeted spell is The End</p>

<button class="btn" on:click={toggleLang}>{lang}</button>
...
<div class="form-control w-full max-w-xs">
    ...
    />
    {/if}
    <button class="btn" on:click={resetSpells}>reset spells</button>
</div>
